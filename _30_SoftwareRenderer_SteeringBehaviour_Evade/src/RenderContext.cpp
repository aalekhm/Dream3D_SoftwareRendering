...
RenderContext::RenderContext(uint32_t iWidth, uint32_t iHeight, uint32_t iPixelWidth, uint32_t iPixelHeight, std::function<void(uint32_t)>* fRenderCallback)
: m_iWidth(iWidth)
, m_iHeight(iHeight)
, m_iPixelWidth(iPixelWidth)
, m_iPixelHeight(iPixelHeight)
, m_fRenderCallback(fRenderCallback)
...
void RenderContext::Render()
{
	...
	{
		if (m_fRenderCallback != nullptr)
		{
			(*m_fRenderCallback)(m_iDeltaTimeMs);
		}
	}
...
void RenderContext::DrawTriangle(Vertex v1, Vertex v2, Vertex v3, Sprite* pBitmapTexture)
{
	...
	{
		FillTriangle3D(v1, v2, v3, pBitmapTexture);
		return;
	}
		...
		ClipPolygonAxis(vVertices, vAuxillaryList, AXIS_COMPONENT::AXIS_Z/* Check for limits on Z-Axis*/))
	{
		...
		{
			FillTriangle3D(vInitialVertex, vVertices.at(i), vVertices.at(i + 1), pBitmapTexture);
		}
	}
}
...
void RenderContext::ClipPolygonComponent(std::vector<Vertex>& vVertices, AXIS_COMPONENT eComponentIndex, float fComponentFactor, std::vector<Vertex>& vResult)
{
	...
	std::vector<Vertex>::iterator it = vVertices.begin();
...
void RenderContext::SetPixel(int destX, int destY, Pixel p, bool bForce)
{
	m_pFrameBuffer->SetPixel(destX, destY, p, bForce);
}
...
void RenderContext::Clear(Pixel p, float fDepth)
{
	...
	{
		m_pFrameBuffer->SetPixel(i%m_iWidth, i / m_iWidth, p, true);
		m_pDepthBuffer[i] = fDepth;
	}
}

void RenderContext::SetFont(Sprite* pFontSprite)
{
	m_pFontSprite = pFontSprite;
}

void RenderContext::DrawString(const char* cStr, int x, int y, int anchor) 
{
	...
	for(int i = 0 ; i < strlen(cStr); i++) 
	{
		...
		if(c > ' ') 
		{
			...
			DrawFont(xX, yY, CHARACTER_WIDTH, CHARACTER_HEIGHT, X + 1, Y);
...
void RenderContext::DrawFont(int xX, int yY, int charW, int charH, int tX, int tY)
{
	FillQuad2D(xX, yY, charW, charH, FONT_TEXTURE_START_X + tX, tY, charW, charH, m_pFontSprite, 1.0f);
}

void RenderContext::FillQuad2D(	float posX, float posY, float posW, float posH, Vector4f vColour)
{
	Vector4f vNormal(0, 0, -1, 0);
	Vector4f vTexCoords;

	Vertex v1(	Vector4f(posX, posY, 0, 1),
				vTexCoords,
				vNormal,
				vColour);
	Vertex v2(	Vector4f(posX + posW, posY, 0, 1),
				vTexCoords,
				vNormal,
				vColour);
	Vertex v3(	Vector4f(posX + posW, posY + posH, 0, 1),
				vTexCoords,
				vNormal,
				vColour);
	Vertex v4(	Vector4f(posX, posY + posH, 0, 1),
				vTexCoords,
				vNormal,
				vColour);

	FillTriangle2D(v1, v2, v3, nullptr);
	FillTriangle2D(v1, v3, v4, nullptr);
}

void RenderContext::FillQuad2D(	float posX, float posY, float posW, float posH,
								float texX, float texY, float texW, float texH,
								Sprite* pTexture, float fAlpha)
{
	Vector4f vNormal(0, 0, -1, 0);
	Vector4f vColour(1, 1, 1, 0);
	int32_t iTexW = pTexture->GetWidth();
	int32_t iTexH = pTexture->GetHeight();
	pTexture->SetTextureAlpha(fAlpha);

	Vertex v1(	Vector4f(posX, posY, 0, 1),
				Vector4f(texX / iTexW, texY / iTexH, 0, 1),
				vNormal,
				vColour);
	Vertex v2(	Vector4f(posX + posW, posY, 0, 1),
				Vector4f((texX + texW) / iTexW, texY / iTexH, 0, 1),
				vNormal,
				vColour);
	Vertex v3(	Vector4f(posX + posW, posY + posH, 0, 1),
				Vector4f((texX + texW) / iTexW, (texY + texH) / iTexH, 0, 1),
				vNormal,
				vColour);
	Vertex v4(	Vector4f(posX, posY + posH, 0, 1),
				Vector4f(texX / iTexW, (texY + texH) / iTexH, 0, 1),
				vNormal,
				vColour);

	FillTriangle2D(v1, v2, v3, pTexture);
	FillTriangle2D(v1, v3, v4, pTexture);
}

void RenderContext::FillTriangle3D(Vertex v1, Vertex v2, Vertex v3, Sprite* pTexture)
{
	Matrix4f identity;
	Matrix4f screenSpaceTransform;
	screenSpaceTransform.initScreenSpaceTransform(GetWidth() / 2, GetHeight() / 2);

	Vertex vMinYVert = v1.Transform(screenSpaceTransform, identity).PerspectiveDivide();
	Vertex vMidYVert = v2.Transform(screenSpaceTransform, identity).PerspectiveDivide();
	Vertex vMaxYVert = v3.Transform(screenSpaceTransform, identity).PerspectiveDivide();

	FillTriangle2D(vMinYVert, vMidYVert, vMaxYVert, pTexture);
}

void RenderContext::FillTriangle2D(Vertex vMinYVert, Vertex vMidYVert, Vertex vMaxYVert, Sprite* pTexture)
{
...
{
	Gradients gradients(vMinYVert, vMidYVert, vMaxYVert, m_fAmbientLightIntensity);
...
void RenderContext::DrawScanLine(Gradients* pGradients, Edge* eLeft, Edge* eRight, int j, Sprite* pTexture)
{
	...
	float fTexCoordXXStep	= pGradients->GetTexCoordXXStep();
	float fTexCoordYXStep	= pGradients->GetTexCoordYXStep();
	float fOneOverZXStep	= pGradients->GetOneOverZXStep();
	float fDepthXStep		= pGradients->GetDepthXStep();
	float fLightAmtStep		= pGradients->GetLightAmtXStep();
	float fColourRStep		= pGradients->GetColourRXStep();
	float fColourGStep		= pGradients->GetColourGXStep();
	float fColourBStep		= pGradients->GetColourBXStep();
	float fColourAStep		= pGradients->GetColourAXStep();
	...
	float fLightAmt		= eLeft->GetLightAmt() + fLightAmtStep * fXPreStep;
	float fColourR		= eLeft->GetColourR() + fColourRStep * fXPreStep;
	float fColourG		= eLeft->GetColourG() + fColourGStep * fXPreStep;
	float fColourB		= eLeft->GetColourB() + fColourBStep * fXPreStep;
	float fColourA		= eLeft->GetColourA() + fColourAStep * fXPreStep;

	int32_t iDepthMax = (m_iWidth * m_iHeight);
	for (int i = xMin; i < xMax; i++)
	{
		int32_t iDepthPos = i + j*m_iWidth;
		if (iDepthPos >= 0 && iDepthPos < iDepthMax && fDepth <= m_pDepthBuffer[iDepthPos])
		{
			m_pDepthBuffer[iDepthPos] = fDepth;
			...
			if (pTexture != nullptr)
			{
				...
				Pixel pResultPixel;
				// Blend Colour & Tex coords.
				{
					Pixel pTexPixel = pTexture->GetPixel(srcX, srcY);
					{
						pTexPixel.m_iRed	= (uint32_t)((pTexPixel.m_iRed & 0xFF) * fLightAmt);
						pTexPixel.m_iGreen	= (uint32_t)((pTexPixel.m_iGreen & 0xFF) * fLightAmt);
						pTexPixel.m_iBlue	= (uint32_t)((pTexPixel.m_iBlue & 0xFF) * fLightAmt);
						pTexPixel.m_iAlpha	= (uint32_t)((pTexPixel.m_iAlpha & 0xFF) * fLightAmt);
					}
					Pixel pColourPixel(	fColourR * 255, fColourG * 255, fColourB * 255, fColourA * 255);
					pResultPixel = Sprite::Blend(pTexPixel, pColourPixel);
				}

				SetPixel(i, j, pResultPixel);
			}
			else
			{
				Pixel pColourPixel(fColourR * 255, fColourG * 255, fColourB * 255, fColourA * 255);
				SetPixel(i, j, pColourPixel);
			}
			...
			fLightAmt	+= pGradients->GetLightAmtXStep();
			fColourR	+= pGradients->GetColourRXStep();
			fColourG	+= pGradients->GetColourGXStep();
			fColourB	+= pGradients->GetColourBXStep();
			fColourA	+= pGradients->GetColourAXStep();
		}
	}
}

void RenderContext::SetAmbientLightIntensity(float fAmbientLightIntensity)
{
	m_fAmbientLightIntensity = fAmbientLightIntensity;
}

float RenderContext::GetAmbientLightIntensity()
{
	return m_fAmbientLightIntensity;
}
