...
void SteerBehaviourUtils::init(RenderContext* pGraphics, Camera* pCamera)
{
	...
	m_iElapsedTimeMs = 0;
	{
		...
		gPursuit2D.m_bEnable = false;
		gPursuit3D.m_bEnable = false;
		gEvade2D.m_bEnable = true;
		gEvade3D.m_bEnable = false;
	}
...
void SteerBehaviourUtils::init2D()
{
	if (gSeek2D.m_bEnable && !gSeek2D.m_bInitialized)
	{
		...
		gSeek2D.m_pTarget = Utils2D::create2DSwastikaShip(Vector4f(0, 1, 1, 0));
		{
			gSeek2D.m_pTarget->SetPosition(Vector3(100, 100, 0));
			gSeek2D.m_pTarget->SetScale(Vector3(0.25, 0.25, 0.25));
		}

		gSeek2D.m_pSeeker = Utils2D::create2DSwastikaShip(Vector4f(0, 0, 0, 0));
		{
	...
	if (gArrive2D.m_bEnable && !gArrive2D.m_bInitialized)
	{
		...
		gArrive2D.m_pTarget = Utils2D::create2DSwastikaShip(Vector4f(0, 1, 1, 0));
		{
			gArrive2D.m_pTarget->SetPosition(Vector3(100, 100, 0));
			gArrive2D.m_pTarget->SetScale(Vector3(0.25, 0.25, 0.25));
		}

		gArrive2D.m_pSeeker = Utils2D::create2DSwastikaShip(Vector4f(0, 0, 0, 0));
		{
	...
	if (gFlee2D.m_bEnable && !gFlee2D.m_bInitialized)
	{
		...
		gFlee2D.m_pTarget = Utils2D::create2DSwastikaShip(Vector4f(0, 1, 1, 0));
		{
			gFlee2D.m_pTarget->SetPosition(Vector3(100, 100, 0));
			gFlee2D.m_pTarget->SetScale(Vector3(0.25, 0.25, 0.25));
		}

		gFlee2D.m_pSeeker = Utils2D::create2DSwastikaShip(Vector4f(0, 0, 0, 0));
		{
	...
	if (gWander2D.m_bEnable && !gWander2D.m_bInitialized)
	{
		...
		gWander2D.m_pSeeker = Utils2D::create2DSwastikaShip(Vector4f(0, 0, 0, 0));
		{
	...
	if (gPursuit2D.m_bEnable && !gPursuit2D.m_bInitialized)
	{
		gPursuit2D.m_bInitialized = true;

		gPursuit2D.m_pTarget = Utils2D::create2DSwastikaShip(Vector4f(1, 1, 1, 1));
		{
			gPursuit2D.m_pTarget->SetPosition(Vector3(100, 100, 0));
			gPursuit2D.m_pTarget->SetScale(Vector3(0.25, 0.25, 0.25));
			ActorSteerBehaviour* pActorSteerBehaviour = (ActorSteerBehaviour*)gPursuit2D.m_pTarget->AddBehaviour(ActorBehaviourType::STEER);
			if (pActorSteerBehaviour != nullptr)
			{
				pActorSteerBehaviour->SetBehaviour(ESteerBehaviour::WANDER);
		...
		gPursuit2D.m_pSeeker = Utils2D::create2DSwastikaShip(Vector4f(0, 0, 0, 1));
		{
			gPursuit2D.m_pSeeker->SetPosition(Vector3(50, 50, 0));
			gPursuit2D.m_pSeeker->SetScale(Vector3(0.25, 0.25, 0.25));
			ActorSteerBehaviour* pActorSteerBehaviour = (ActorSteerBehaviour*)gPursuit2D.m_pSeeker->AddBehaviour(ActorBehaviourType::STEER);
			if (pActorSteerBehaviour != nullptr)
			{
				pActorSteerBehaviour->SetBehaviour(ESteerBehaviour::PURSUIT);
				pActorSteerBehaviour->SetTarget(ESteerBehaviour::PURSUIT, gPursuit2D.m_pTarget);
			}
		}
	}

	if (gEvade2D.m_bEnable && !gEvade2D.m_bInitialized)
	{
		gEvade2D.m_bInitialized = true;

		gEvade2D.m_pTarget = Utils2D::create2DSwastikaShip(Vector4f(1, 1, 1, 1));
		{
			gEvade2D.m_pTarget->SetPosition(Vector3(100, 100, 0));
			gEvade2D.m_pTarget->SetScale(Vector3(0.25, 0.25, 0.25));
			ActorSteerBehaviour* pActorSteerBehaviour = (ActorSteerBehaviour*)gEvade2D.m_pTarget->AddBehaviour(ActorBehaviourType::STEER);
			if (pActorSteerBehaviour != nullptr)
			{
				pActorSteerBehaviour->SetBehaviour(ESteerBehaviour::WANDER);
				pActorSteerBehaviour->SetTarget(ESteerBehaviour::WANDER, nullptr);
		...
		gEvade2D.m_pEvader = Utils2D::create2DSwastikaShip(Vector4f(0, 0, 0, 1));
		{
			gEvade2D.m_pEvader->SetPosition(Vector3(50, 50, 0));
			gEvade2D.m_pEvader->SetScale(Vector3(0.25, 0.25, 0.25));
			ActorSteerBehaviour* pActorSteerBehaviour = (ActorSteerBehaviour*)gEvade2D.m_pEvader->AddBehaviour(ActorBehaviourType::STEER);
			if (pActorSteerBehaviour != nullptr)
			{
				pActorSteerBehaviour->SetBehaviour(ESteerBehaviour::EVADE);
				pActorSteerBehaviour->SetTarget(ESteerBehaviour::EVADE, gEvade2D.m_pTarget);
			}
		}
	}
}
...
void SteerBehaviourUtils::init3D()
{
	...
	if (gEvade3D.m_bEnable && !gEvade3D.m_bInitialized)
	{
		gEvade3D.m_bInitialized = true;

		gEvade3D.m_pTarget = new Actor3D("../Content/Arrow.obj", "../Content/sp3bot.tga");
		{
			gEvade3D.m_pTarget->SetScale(Vector3(0.1f, 0.1f, 0.2f));
			gEvade3D.m_pTarget->SetPosition(Vector3(0, 0, 0));
			gEvade3D.m_pTarget->SetCamera(m_pCamera);
			ActorSteerBehaviour* pActorSteerBehaviour = (ActorSteerBehaviour*)gEvade3D.m_pTarget->AddBehaviour(ActorBehaviourType::STEER);
			if (pActorSteerBehaviour != nullptr)
			{
				pActorSteerBehaviour->SetBehaviour(ESteerBehaviour::WANDER);
				pActorSteerBehaviour->SetTarget(ESteerBehaviour::WANDER, nullptr);
		...
		gEvade3D.m_pEvader = new Actor3D("../Content/Arrow.obj", "../Content/test.tga");
		{
			gEvade3D.m_pEvader->SetScale(Vector3(0.1f, 0.1f, 0.2f));
			gEvade3D.m_pEvader->SetPosition(Vector3(0, 0, 0));
			gEvade3D.m_pEvader->SetCamera(m_pCamera);
			ActorSteerBehaviour* pActorSteerBehaviour = (ActorSteerBehaviour*)gEvade3D.m_pEvader->AddBehaviour(ActorBehaviourType::STEER);
			if (pActorSteerBehaviour != nullptr)
			{
				pActorSteerBehaviour->SetBehaviour(ESteerBehaviour::EVADE);
				pActorSteerBehaviour->SetTarget(ESteerBehaviour::EVADE, gEvade3D.m_pTarget);
			}
		}
	}
}
...
void SteerBehaviourUtils::render2D(int32_t iDeltaTimeMs)
{
	...
	if (gPursuit2D.m_bEnable)
	{
		pActorSteerBehaviour = (ActorSteerBehaviour*)gPursuit2D.m_pTarget->GetBehaviour(ActorBehaviourType::STEER);
		if (pActorSteerBehaviour != nullptr)
		{
			Vector3 vVelocity = pActorSteerBehaviour->GetVelocity();

			Vector3 vLookAt = gPursuit2D.m_pTarget->GetPosition() + (vVelocity * 2);
			gPursuit2D.m_pTarget->LookAt(vLookAt);

			check2DBounds(gPursuit2D.m_pTarget);
			gPursuit2D.m_pTarget->Update(m_pGraphics, iDeltaTimeMs);
		}

		pActorSteerBehaviour = (ActorSteerBehaviour*)gPursuit2D.m_pSeeker->GetBehaviour(ActorBehaviourType::STEER);
		if (pActorSteerBehaviour != nullptr)
		{
			Vector3 vVelocity = pActorSteerBehaviour->GetVelocity();

			Vector3 vLookAt = gPursuit2D.m_pSeeker->GetPosition() + (vVelocity * 2);
			gPursuit2D.m_pSeeker->LookAt(vLookAt);

			check2DBounds(gPursuit2D.m_pSeeker);

			gPursuit2D.m_pSeeker->Update(m_pGraphics, iDeltaTimeMs);
		}
	}

	if (gEvade2D.m_bEnable)
	{
		pActorSteerBehaviour = (ActorSteerBehaviour*)gEvade2D.m_pTarget->GetBehaviour(ActorBehaviourType::STEER);
		if (pActorSteerBehaviour != nullptr)
		{
			Vector3 vVelocity = pActorSteerBehaviour->GetVelocity();

			Vector3 vLookAt = gEvade2D.m_pTarget->GetPosition() + (vVelocity * 2);
			gEvade2D.m_pTarget->LookAt(vLookAt);

			check2DBounds(gEvade2D.m_pTarget);
			gEvade2D.m_pTarget->Update(m_pGraphics, iDeltaTimeMs);
		}

		pActorSteerBehaviour = (ActorSteerBehaviour*)gEvade2D.m_pEvader->GetBehaviour(ActorBehaviourType::STEER);
		if (pActorSteerBehaviour != nullptr)
		{
			Vector3 vVelocity = pActorSteerBehaviour->GetVelocity();

			Vector3 vLookAt = gEvade2D.m_pEvader->GetPosition() + (vVelocity * 2);
			gEvade2D.m_pEvader->LookAt(vLookAt);

			check2DBounds(gEvade2D.m_pEvader);

			gEvade2D.m_pEvader->Update(m_pGraphics, iDeltaTimeMs);
		}
	}
}
...
void SteerBehaviourUtils::render3D(int32_t iDeltaTimeMs)
{
	...
	if (gEvade3D.m_bEnable)
	{
		ActorSteerBehaviour* pActorSteerBehaviour = (ActorSteerBehaviour*)gEvade3D.m_pTarget->GetBehaviour(ActorBehaviourType::STEER);
		if (pActorSteerBehaviour != nullptr)
		{
			Vector3 vVelocity = pActorSteerBehaviour->GetVelocity();
			Vector3 vLookAt = gEvade3D.m_pTarget->GetPosition() + (vVelocity * 1);
			const float EPSILON = 0.00000001f;
			if (vLookAt.length() > EPSILON)
			{
				gEvade3D.m_pTarget->LookAt(vLookAt);
			}
		}

		check3DBounds(gEvade3D.m_pTarget);
		gEvade3D.m_pTarget->Update(m_pGraphics, iDeltaTimeMs);

		pActorSteerBehaviour = (ActorSteerBehaviour*)gEvade3D.m_pEvader->GetBehaviour(ActorBehaviourType::STEER);
		if (pActorSteerBehaviour != nullptr)
		{
			Vector3 vVelocity = pActorSteerBehaviour->GetVelocity();
			Vector3 vLookAt = gEvade3D.m_pEvader->GetPosition() + (vVelocity * 1);
			const float EPSILON = 0.00000001f;
			if (vLookAt.length() > EPSILON)
			{
				gEvade3D.m_pEvader->LookAt(vLookAt);
			}
		}

		check3DBounds(gEvade3D.m_pEvader);
		gEvade3D.m_pEvader->Update(m_pGraphics, iDeltaTimeMs);
	}
}
...
void SteerBehaviourUtils::keyReleasedEx(unsigned int iVirtualKeycode, unsigned short ch)
{
	if (ch == 'T' || ch == 't')
	{
		...
		gPursuit2D.m_bEnable = false;
		gPursuit3D.m_bEnable = false;
		gEvade2D.m_bEnable = m_b2DToggle;
		gEvade3D.m_bEnable = !m_b2DToggle;
