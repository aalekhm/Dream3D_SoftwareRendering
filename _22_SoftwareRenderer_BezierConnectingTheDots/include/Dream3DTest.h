...
class Dream3DTest : EngineManager
{
	...
	protected:
		...
		Vector4f						GetBeizerPoint(std::vector<Vector4f>& vControlPoints, int32_t iStartPoint, int32_t iEndPoint, float fTime);

		Mesh*							GenerateBezierSurface();
		double							BezierBlend(int k, float t, int n);

		void							Draw2DBezierCurveFromPointList(uint32_t iDeltaTimeMs, float fElapsedTimeMs, std::vector<Vector4f>& vBeizerCurvePathPoints2DOriginalList, std::vector<Vector4f>& vBeizerCurvePathPoints2DListGenerated);
		void							Generate2DBezierCurvePoints(std::vector<Vector4f>& vInititalList, std::vector<Vector4f>& vFinalList);
	private:
