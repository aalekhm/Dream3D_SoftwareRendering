...
bool createGLWindow(char* title, unsigned int width, unsigned int height, unsigned int pixelW, unsigned int pixelH, int bits, bool isFullScreen) {
	int			iPixelFormat;
	WNDCLASS	wc;
	DWORD		dwExStyle;
	DWORD		dwStyle;
	...
	windowRect.right	= (long)width * pixelW;
	windowRect.top		= (long)0;
	windowRect.bottom	= (long)height * pixelH;
...
int winMainLoop() {
	...
	m_pEngineManager = EngineManager::getInstance();
	{
		if (!createGLWindow(	"Dream3D!!!", 
								...
								m_pEngineManager->getPixelWidth(),
								m_pEngineManager->getPixelWidth(),
								16, m_bFULLSCREEN))
			return 0;
		...
		if(PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			if(EngineManager::isKeyPressed(VK_F1)) {
				...
				if(!createGLWindow(	"Dream3D!!!", 
									...
									m_pEngineManager->getHeight(), 
									m_pEngineManager->getPixelWidth(), 
									m_pEngineManager->getPixelHeight(), 
									16, m_bFULLSCREEN)) 
				{
					return 0;
				}
