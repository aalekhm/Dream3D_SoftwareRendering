...
bool					gShow2DBezierCurvePath2D = false;
std::vector<Vector4f>	gvBeizerCurvePathPoints2DList;
std::vector<Vector4f>	gvBeizerCurvePathPoints2DListGenerated;

bool					gShowBezierCurvePath3D = true;
bool					gShowBezierCurvePath3D_CONSTANT_Y = false;
std::vector<Mesh*>		gvControlPointMeshes;
std::vector<Mesh*>		gvIntermediatePointMeshes;
std::vector<Transform*>	gvControlPointMeshesTransform;
std::vector<Transform*>	gvIntermediatePointMeshesTransform;
std::vector<Vector4f>	gvBeizerCurvePathPoints3DList;
std::vector<Vector4f>	gvBeizerCurvePathPoints3DListGenerated;
Actor3D*				gShapeSpaceShip3D;
...
void Dream3DTest::init3D()
{
	gCamera = new Camera(	Matrix4f().initPerspective(	(float)(70.0f * (3.142f / 180.0f)),
													...
													0.1f,
													1000.0f));
	gCamera->move(Vector4f(0, 1, 0), 1);
	gCamera->move(Vector4f(0, 0, -5), 1);
	...
	if (gShowBezierCurvePath3D)
	{
		gvBeizerCurvePathPoints3DList.push_back(Vector4f(0.0,	1.0,	0.0,	1));
		gvBeizerCurvePathPoints3DList.push_back(Vector4f(0.5,	-0.5,	-1.5,	1));
		gvBeizerCurvePathPoints3DList.push_back(Vector4f(-1.0,	0.5,	-2.5,	1));
		gvBeizerCurvePathPoints3DList.push_back(Vector4f(1.0,	-1.0,	-0.5,	1));
		gvBeizerCurvePathPoints3DList.push_back(Vector4f(0.5,	-0.0,	1.5,	1));
		gvBeizerCurvePathPoints3DList.push_back(Vector4f(-1.0,	0.5,	-3.0,	1));
		gvBeizerCurvePathPoints3DList.push_back(Vector4f(-2.0,	0.0,	-0.5,	1));

		GenerateBezierCurvePoints(gvBeizerCurvePathPoints3DList, gvBeizerCurvePathPoints3DListGenerated);

		gShapeSpaceShip3D = new Actor3D("../Content/Arrow.obj", "../Content/sp3bot.tga");
		gShapeSpaceShip3D->SetScale(Vector3(0.1f, 0.1f, 0.2f));
		gShapeSpaceShip3D->SetCamera(gCamera);

		// Control Points
		for (int32_t i = 0; i < gvBeizerCurvePathPoints3DList.size(); i++)
		{
			Mesh* pCubeMesh = new Mesh("../Content/cube.obj");
			gvControlPointMeshes.push_back(pCubeMesh);

			Transform* pCubeTransform = new Transform(	gvBeizerCurvePathPoints3DList[i],
														Quaternion(0, 0, 0, 1),
														Vector4f(0.1f, 0.1f, 0.1f)
													);
			gvControlPointMeshesTransform.push_back(pCubeTransform);
		}

		// Points in-between the Control Points
		int32_t iStartControlPoint = 0, iEndControlPoint = 0;
		{
			int32_t iInterval = gvBeizerCurvePathPoints3DListGenerated.size() / 3;
			...
			for (float fT = 0.05f; fT < 1.0f * iInterval; fT += 0.05f)
			{
				...
				vBeizerPt = GetBeizerPoint(gvBeizerCurvePathPoints3DListGenerated, iStartControlPoint, iEndControlPoint, fTT);

				Mesh* pCubeMesh = new Mesh("../Content/cube.obj");
				gvIntermediatePointMeshes.push_back(pCubeMesh);
				...
				gvIntermediatePointMeshesTransform.push_back(pCubeTransform);
			}
		}
	}
}
...
void Dream3DTest::init2D()
{
	...
	if (gShow2DBezierCurvePath2D)
	{
		gvBeizerCurvePathPoints2DList.push_back(Vector4f(50,	200,	0, 0));
		gvBeizerCurvePathPoints2DList.push_back(Vector4f(100,	100,	0, 0));
		gvBeizerCurvePathPoints2DList.push_back(Vector4f(150,	200,	0, 0));
		gvBeizerCurvePathPoints2DList.push_back(Vector4f(200,	100,	0, 0));
		gvBeizerCurvePathPoints2DList.push_back(Vector4f(100,	 50,	0, 0));
		gvBeizerCurvePathPoints2DList.push_back(Vector4f(200,	 50,	0, 0));
		gvBeizerCurvePathPoints2DList.push_back(Vector4f(250,	100,	0, 0));
		gvBeizerCurvePathPoints2DList.push_back(Vector4f(300,	 50,	0, 0));
		gvBeizerCurvePathPoints2DList.push_back(Vector4f(350,	100,	0, 0));
		gvBeizerCurvePathPoints2DList.push_back(Vector4f(350,	250,	0, 0));
		gvBeizerCurvePathPoints2DList.push_back(Vector4f(200,	250,	0, 0));
		gvBeizerCurvePathPoints2DList.push_back(Vector4f( 50,	200,	0, 0));

		GenerateBezierCurvePoints(gvBeizerCurvePathPoints2DList, gvBeizerCurvePathPoints2DListGenerated);
	}
}
...
void Dream3DTest::render3D(uint32_t iDeltaTimeMs)
{
	...
	Matrix4f viewProjMat = gCamera->getViewProjection();
	...
	if(gShow3DBezierAnimation)
	{
		...
		{
			DrawMesh(m_pGraphics, gBezierControlPoints3D[i], gBezierControlPoints3DTransform[i], viewProjMat, gBrickTexture);
		...
		float t = abs(sinf(gElapsedTimeCounter));
		{
			...
			DrawMesh(m_pGraphics, gBezierPoint3D, &gBezierPoint3DTransform, mm, gBrickTexture);
		}
	}
	
	if(gShow3DBezierSurface)
	{
		DrawMesh(m_pGraphics, gBezierSurfaceMesh, &gBezierSurfaceMeshTransform, viewProjMat, gCartoon);
	}

	if (gShowBezierCurvePath3D)
	{
		DrawBezierCurveFromPointList(false, iDeltaTimeMs, (gElapsedTimeCounter - gForceResetTimeCounter), gvBeizerCurvePathPoints3DList, gvBeizerCurvePathPoints3DListGenerated);
	}
}
...
void Dream3DTest::render2D(uint32_t iDeltaTimeMs)
{
	...
	if (gShow2DBezierCurvePath2D)
	{
		DrawBezierCurveFromPointList(true, iDeltaTimeMs, (gElapsedTimeCounter - gForceResetTimeCounter), gvBeizerCurvePathPoints2DList, gvBeizerCurvePathPoints2DListGenerated);
	}
...
void Dream3DTest::DrawBezierCurveFromPointList(	bool bIs2D,
												...
												std::vector<Vector4f>& vBeizerCurvePathPointsListGenerated)
{
	...
	if (!bIs2D)
	{
		matViewProj = gCamera->getViewProjection();
	}
	...
	int32_t iStartControlPoint = 0, iEndControlPoint = 0;
	{
		...
		for (float fT = 0.05f; fT < 1.0f * iInterval; fT += 0.05f, iCount++)
		{
			...
			vBeizerPt = GetBeizerPoint(vBeizerCurvePathPointsListGenerated, iStartControlPoint, iEndControlPoint, fTT);

			if(bIs2D)
				m_pGraphics->SetPixel(vBeizerPt.getX(), vBeizerPt.getY(), Pixel(255, 0, 255, 255), true);
			else
			{
				DrawMesh(m_pGraphics, gvIntermediatePointMeshes[iCount], gvIntermediatePointMeshesTransform[iCount], matViewProj, gBrickTexture2);
			}
		}
		...
		{
			vBeizerPt = GetBeizerPoint(vBeizerCurvePathPointsListGenerated, iStartControlPoint, iEndControlPoint, (t - iStart* fTInterval) * iInterval);
			Vector4f vLastBeizerPt = GetBeizerPoint(vBeizerCurvePathPointsListGenerated, iStartLast * 3, (iStartLast * 3) + 4, (gLastTMs - iStartLast* fTInterval) * iInterval);
			{
				if (bIs2D)
				{
					m_pGraphics->SetPixel(vBeizerPt.getX(), vBeizerPt.getY(), Pixel(255, 255, 255, 255), true);
				...
				{					
					gShapeSpaceShip3D->SetPosition(Vector3(vLastBeizerPt.x, vLastBeizerPt.y, vLastBeizerPt.z));

					if (gShowBezierCurvePath3D_CONSTANT_Y)
					{
						Vector4f vLookAt4f = ((vBeizerPt.sub(vLastBeizerPt)).normalized()).mul(1);
						vLookAt4f.add(vLastBeizerPt);
						gShapeSpaceShip3D->LookAtConstantY(Vector3(vLookAt4f.x, vLookAt4f.y, vLookAt4f.z));
					}
					else
						gShapeSpaceShip3D->LookAt(Vector3(vBeizerPt.x, vBeizerPt.y, vBeizerPt.z));
					
					gShapeSpaceShip3D->Update(m_pGraphics, iDeltaTimeMs);
				}
	...
	for (int32_t i = 0; i < vBeizerCurvePathPointsListGenerated.size(); i++)
	{
		for (int32_t j = 0; j < vBeizerCurvePathPointsOriginalList.size(); j++)
		{
			...
			if (vOriginal.getX() == vGenerated.getX()
				...
				{
					DrawMesh(m_pGraphics, gvControlPointMeshes[j], gvControlPointMeshesTransform[j], matViewProj, gBrickTexture);
				}
			}
...
void Dream3DTest::GenerateBezierCurvePoints(std::vector<Vector4f>& vInititalList, std::vector<Vector4f>& vFinalList)
{
