...
, m_iPixelHeight(iPixelHeight)
{
	...
	gMonkeyMesh = new Mesh("../Content/monkey2.obj");
...
void RenderContext::RenderInternal(uint32_t iDeltaTimeMs)
{
	...
	{
		gCamera->update(iDeltaTimeMs / 1000.0f);
		Matrix4f vp = gCamera->getViewProjection();

		gTerrainMesh->Draw(this, vp.mul(gTerrainTransform.getTransformation()), gBrickTexture);
		gMonkeyMesh->Draw(this, vp.mul(gMonkeyTransform.getTransformation()), gBrickTexture2);
	}
}
