...
class RenderContext
{
	...
	protected:
		...
		void												DrawScanBuffer(int32_t iYCoord, int32_t iXMin, int32_t iXMax);
		void												FillShape(int32_t iYMin, int32_t iYMax);
		void												ScanConvertTriangle(Vertex vMinVert, Vertex vMidVert, Vertex vMaxVert, int32_t iHandedness);
		void												ScanConvertLine(Vertex vMinYVert, Vertex vMaxYVert, int32_t iWhichSide);
	private:
