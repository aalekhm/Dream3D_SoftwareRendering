...
Vertex::Vertex(Vector4f vPos, Vector4f vTexCoords, Vector4f vNormal, Vector4f vColour)
{
	m_vPos			= vPos;
...
float Vertex::GetX() {	return m_vPos.getX(); }
float Vertex::GetY() {	return m_vPos.getY(); }
float Vertex::GetZ() {	return m_vPos.getZ(); }

float Vertex::GetR() { return m_vColour.getX(); }
float Vertex::GetG() { return m_vColour.getY(); }
float Vertex::GetB() { return m_vColour.getZ(); }
float Vertex::GetA() { return m_vColour.getW(); }

Vector4f Vertex::GetPosition() {	return m_vPos; }
Vector4f Vertex::GetTexCoords() {	return m_vTexCoords; }
Vector4f Vertex::GetNormal() {		return m_vNormal; }
Vector4f Vertex::GetColour() {		return m_vColour; }

void Vertex::SetX(float fX) {	m_vPos.x = fX; }
void Vertex::SetY(float fY) {	m_vPos.y = fY; }
void Vertex::SetZ(float fZ) {	m_vPos.z = fZ; }

void Vertex::SetR(float fX) { m_vColour.x = fX; }
void Vertex::SetG(float fY) { m_vColour.y = fY; }
void Vertex::SetB(float fZ) { m_vColour.z = fZ; }
void Vertex::SetA(float fW) { m_vColour.w = fW; }
...
{
	return Transform(transform, Matrix4f());
}

Vertex Vertex::Transform(Matrix4f transform, Matrix4f matNormalTransform)
{
	return Vertex(	transform.transform(m_vPos), m_vTexCoords, matNormalTransform.transform(m_vNormal).normalized(), m_vColour );
}
...
Vertex Vertex::Lerp(Vertex vOther, float fLerpAmt)
{
	return Vertex(	m_vPos.lerp(vOther.GetPosition(), fLerpAmt),
					...
					m_vColour.lerp(vOther.GetColour(), fLerpAmt));
}
