...
void RenderContext::DrawScanLine(Gradients* pGradients, Edge* eLeft, Edge* eRight, int j, Sprite* pTexture)
{
	...
	float fTexCoordX = eLeft->GetTexCoordX() + pGradients->GetTexCoordXXStep() * fXPreStep;
	float fTexCoordY = eLeft->GetTexCoordY() + pGradients->GetTexCoordYXStep() * fXPreStep;

	for (int i = xMin; i < xMax; i++)
	{
		int srcX = (int)(fTexCoordX * (pTexture->GetWidth() - 1) + 0.5f);
		int srcY = (int)(fTexCoordY * (pTexture->GetHeight() - 1) + 0.5f);
