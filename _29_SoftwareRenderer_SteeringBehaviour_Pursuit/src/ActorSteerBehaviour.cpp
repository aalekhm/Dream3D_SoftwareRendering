...
Vector3 ActorSteerBehaviour::Calculate(int32_t iDeltaTimeMs)
{
	...
	switch (m_eSummingMethod)
	{
		...
		{
			m_vForce = CalculateWeightedSum(iDeltaTimeMs);
		}
		break;
		case ESteerSummingMethod::PRIORITIZED:
		{
			m_vForce = CalculatePrioritized(iDeltaTimeMs);
		}
		break;
		case ESteerSummingMethod::DITHERED:
		{
			m_vForce = CalculateDithered(iDeltaTimeMs);
		}
		break;
	}
...
Vector3 ActorSteerBehaviour::CalculateWeightedSum(int32_t iDeltaTimeMs)
{
	...
	{
		m_vForce += Wander(iDeltaTimeMs) * m_fWeightedSum[ESteerBehaviour::WANDER];
		m_vForce.truncate(m_fMaxForce);
	}

	if (IsOn(ESteerBehaviour::PURSUIT))
	{
		pTarget = m_vTarget[ESteerBehaviour::PURSUIT];
		if (pTarget != nullptr)
		{
			v3TargetPos = pTarget->GetPosition();

			m_vForce += Pursuit(pTarget) * m_fWeightedSum[ESteerBehaviour::PURSUIT];
			m_vForce.truncate(m_fMaxForce);
		}
	}
