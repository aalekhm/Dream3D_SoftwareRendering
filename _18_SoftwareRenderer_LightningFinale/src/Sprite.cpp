...
void Sprite::SetPixel(int32_t x, int32_t y, Pixel p, float fLightAmt)
{
	Pixel pP;
	pP.m_iRed		= (uint8_t)((p.m_iRed & 0xFF)	* fLightAmt);
	pP.m_iGreen		= (uint8_t)((p.m_iGreen & 0xFF) * fLightAmt);
	pP.m_iBlue		= (uint8_t)((p.m_iBlue & 0xFF)	* fLightAmt);
	pP.m_iAlpha		= 0xFF;

	m_pColData[y*m_iWidth + x] = pP;
}
