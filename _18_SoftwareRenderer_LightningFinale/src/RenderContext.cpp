...
void RenderContext::RenderInternal(uint32_t iDeltaTimeMs)
{
	Clear(Pixel(0, 0, 0, 0xFF), FLT_MAX);
	{
		...
		gTerrainMesh->Draw(this, vp, gTerrainTransform.getTransformation(), gBrickTexture);
		gMonkeyMesh->Draw(this, vp, gMonkeyTransform.getTransformation(), gBrickTexture2);
	}
}
...
void RenderContext::DrawPixel(int32_t iX, int32_t iY, float fLightAmt, Pixel pPixel)
{
	m_pFrameBuffer->SetPixel(iX, iY, pPixel, fLightAmt);
}

void RenderContext::CopyPixel(int destX, int destY, int srcX, int srcY, float fLightAmt, Sprite* pTexture)
{
	m_pFrameBuffer->SetPixel(destX, destY, pTexture->GetPixel(srcX, srcY), fLightAmt);
}
...
void RenderContext::Clear(Pixel p, float fDepth)
{
	...
	{
		m_pFrameBuffer->SetPixel(i%m_iWidth, i / m_iWidth, p, 1.0f);
...
void RenderContext::DrawScanLine(Gradients* pGradients, Edge* eLeft, Edge* eRight, int j, Sprite* pTexture)
{
	...
	//float fLightAmtXStep = (eRight->GetLightAmt() - eLeft->GetLightAmt()) / fXDist;
	...
	for (int i = xMin; i < xMax; i++)
	{
		if (fDepth < m_pDepthBuffer[i + j*m_iWidth])
		{
			...
			CopyPixel(i, j, srcX, srcY, fLightAmt, pTexture);
