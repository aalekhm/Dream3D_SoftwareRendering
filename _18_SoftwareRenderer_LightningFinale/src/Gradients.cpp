...
Gradients::Gradients(Vertex vMinYVert, Vertex vMidYVert, Vertex vMaxYVert)
{
	...
	float fAmbientLightEffect = 0.2f;
	float fLightIntensity = 0.9f;
	Vector4f lightDir(0, 0, 1, 0);
	m_fLightAmt[0] = Saturate(vMinYVert.GetNormal().dot(lightDir)) * fLightIntensity + fAmbientLightEffect;
	m_fLightAmt[1] = Saturate(vMidYVert.GetNormal().dot(lightDir)) * fLightIntensity + fAmbientLightEffect;
	m_fLightAmt[2] = Saturate(vMaxYVert.GetNormal().dot(lightDir)) * fLightIntensity + fAmbientLightEffect;
...
float Gradients::GetDepth(int32_t iLoc)
{
	return m_fDepth[iLoc];
}

float Gradients::GetLightAmt(int32_t iLoc)
{
	return m_fLightAmt[iLoc];
}
...
{
	return m_fDepthXStep;
}

float Gradients::GetDepthYStep()
{
	return m_fDepthYStep;
}

float Gradients::GetLightAmtXStep()
{
	return m_fLightAmtXStep;
}

float Gradients::GetLightAmtYStep()
{
	return m_fLightAmtYStep;
}
