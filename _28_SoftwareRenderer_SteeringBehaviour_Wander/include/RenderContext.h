...
class RenderContext
{
	public:
		...
		void												FillQuad2D(	float posX, float posY, float posW, float posH, Vector4f vColour);
		void												FillQuad2D(	float posX, float posY, float posW, float posH,
																		float texX, float texY, float texW, float texH,
																		Sprite* pTexture, float fAlpha);

		void												SetFont(Sprite* pFontSprite);
		void												DrawString(const char* cStr, int x, int y, int anchor);
