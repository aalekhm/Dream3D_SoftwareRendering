...
#define BOTTOM_HUD_CONTENT_X	10
#define BOTTOM_HUD_CONTENT_Y	BOTTOM_HUD_HEIGHT + 2
...
void Dream3DTest::initWorld()
{
	...
	SteerBehaviourUtils::init(m_pGraphics, m_pCamera);
}
...
void Dream3DTest::onRender(uint32_t iDeltaTimeMs)
{
	...
	SteerBehaviourUtils::render(iDeltaTimeMs);
}
...
void Dream3DTest::render2D(uint32_t iDeltaTimeMs)
{
	...
	DrawStatusBar();
	{
		...
		m_pGraphics->DrawString("Press 'T'/'t' to toggle between 2D/3D.", BOTTOM_HUD_CONTENT_X, pEngineManager->getHeight() - BOTTOM_HUD_CONTENT_Y, 0);
		m_pGraphics->DrawString("Left Click to Change(3D)/Set(2D) target position.", BOTTOM_HUD_CONTENT_X, pEngineManager->getHeight() - BOTTOM_HUD_CONTENT_Y + 15, 0);
	}
}
...
void Dream3DTest::DrawStatusBar()
{
	...
	int32_t iTop = pEngineManager->getHeight() - BOTTOM_HUD_HEIGHT;

	m_pGraphics->FillQuad2D(0, iTop, pEngineManager->getWidth(), BOTTOM_HUD_HEIGHT, Vector4f(0, 0, 0, 1));
...
{
	SteerBehaviourUtils::keyReleasedEx(iVirtualKeycode, ch);
}
...
{
	SteerBehaviourUtils::onMouseUpEx(mCode, x, y);
}
