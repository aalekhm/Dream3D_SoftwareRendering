...
public:
	...
	void		transformVector2Ds(std::vector<Vector2> &vPoint);
	void		transformVector2D(Vector2 &vTransPoint);
...
public:
	...
	void		rotate(const Vector3 &fwd, const Vector3 &side, const Vector3 &up);
...
inline void Matrix3::scale(double xScale, double yScale) {
	Matrix3 mat;

	mat[0] = xScale; mat[3] = 0; mat[6] = 0;

	mat[1] = 0; mat[4] = yScale; mat[7] = 0;
...
inline void Matrix3::rotate(const Vector2 &fwd, const Vector2 &side)
{
	Matrix3 mat;

	mat[0] = fwd.x; mat[3] = side.x; mat[6] = 0;

	mat[1] = fwd.y; mat[4] = side.y; mat[7] = 0;
...
inline void Matrix3::translate(double x, double y)
{
	Matrix3 mat;
...
inline void Matrix3::transformVector2Ds(std::vector<Vector2> &vPoint)
{
	for (unsigned int i = 0; i < vPoint.size(); ++i)
	{
		double tempX = (m[0]*vPoint[i].x) + (m[3]*vPoint[i].y) + (m[6]);

		double tempY = (m[1]*vPoint[i].x) + (m[4]*vPoint[i].y) + (m[7]);
...
//applies a 2D transformation matrix to a std::vector of Vector2Ds
inline void Matrix3::transformVector2D(Vector2 &vTransPoint)
{
	double tempX = (m[0]*vTransPoint.x) + (m[3]*vTransPoint.y) + (m[6]);
	double tempY = (m[1]*vTransPoint.x) + (m[4]*vTransPoint.y) + (m[7]);
	vTransPoint.x = tempX;
	vTransPoint.y = tempY;
}
...
inline Vector3 Matrix4::getColumn(int columnNo)
{
	Vector3 retVector;
...
inline Vector3 Matrix4::getRow(int rowNo)
{
	Vector3 retVector;
