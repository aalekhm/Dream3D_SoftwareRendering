...
void RenderContext::FillTriangle(Vertex v1, Vertex v2, Vertex v3)
{
	...
	FillShape((int32_t)ceil(vMinYVert.GetY()), (int32_t)ceil(vMaxYVert.GetY()));
}
...
{
	int32_t iYStart	= (int32_t)ceil(vMinYVert.GetY());
	int32_t iYEnd	= (int32_t)ceil(vMaxYVert.GetY());
	int32_t iXStart	= (int32_t)ceil(vMinYVert.GetX());
	int32_t iXEnd	= (int32_t)ceil(vMaxYVert.GetX());

	float iXDist = vMaxYVert.GetX() - vMinYVert.GetX();
	float iYDist = vMaxYVert.GetY() - vMinYVert.GetY();
	if (iYDist < 0)
	{
		return;
	}
	...
	float fYPreStep = iYStart - vMinYVert.GetY();
	float fCurX = vMinYVert.GetX() + (fYPreStep * fXStep);

	for (int32_t j = iYStart; j < iYEnd; j++)
	{
		m_iScanBuffer[j * 2 + iWhichSide] = (int32_t)ceil(fCurX);
		fCurX += fXStep;
	}
}
